#ifndef _SIMPLE_ORAM_H_
#define _SIMPLE_ORAM_H_
...
class SimpleORAM : public ORAM {
private:
  uint64_t pos_map_capacity_;
  SimpleORAM* pos_map_; // Used only when pos_map_capacity > MAX_POS_MAP.
  MemTree tree_;
  uint8_t *pos_map_block_;
  uint8_t *fetched_block_;
  bool AccessSimpleORAM(
...
public:
  ...
  explicit SimpleORAM(uint64_t capacity, uint64_t block_size);

  ~SimpleORAM();

  SimpleORAM(const SimpleORAM&) = delete;
  SimpleORAM& operator=(const SimpleORAM&) = delete;
